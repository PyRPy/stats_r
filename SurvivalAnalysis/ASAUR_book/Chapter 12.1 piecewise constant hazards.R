...
# 12.1 piecewise constant hazards -----------------------------------------
...
simple.split.s <- survSplit(Surv(tt, delta) ~ ., 
                            data=simple, cut=tau.s, end="tt",
                            start="t0", event="delta", 
                            episode="diagGrp")
...
# note new argument:  Surv(survTime, status) ~ .    
# as required by update in survival package

prost.80plus.poor$survTime[prost.80plus.poor$survTime == 0] <- 0.1  
# minimum time must be > 0 for survSplit
prost.split.s <- survSplit(Surv(survTime, status.all) ~ ., 
                           data=prost.80plus.poor, cut=tau.s,
                          end="survTime", start="t0", 
