...
# The following codes are not tested --------------------------------------
...
#  survfit(Surv(survTime, status, type="mstate") ~ 1, 
#            data=prostateSurvival.highrisk)
# as described below
# # # # # # # # # # # # # # # # # # # # # # # # # # # #  ! !
# # # # # # # # # # # # # # # # # # # # # # # # # # # #  ! !
...
# figure 9.4   (simple)       
# works for R version 3.2, perhaps not for version 3.3
windows()
...
#par(mar=c(5, 4, 4, 4) + 0.1)
plot(cumDist.prost.km ~ time.km, type="s", ylim=c(0,1), lwd=1, 
     xlab="Years from prostate cancer diagnosis",
...
plot(ci1 ~ times, type="s", ylim=c(0,1), lwd=2, 
     xlab="Years from prostate cancer diagnosis",
...
summary(coxph(Surv(time, status) ~ grade*factor(trans) + ageGroup + strata(trans), 
              data=prostate.long))  
summary(coxph(Surv(time, status) ~ (grade + ageGroup)*trans + ageGroup + strata(trans), 
              data=prostate.long)) 
